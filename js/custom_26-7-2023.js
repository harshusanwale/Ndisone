$(document).ready(function () {
    ...
    $(".lis-wor-his-add-btn").on('click', function () {
        ...
        $(".add-list-ser ul li:last-child").after('<li><div class="row"><div class="col-md-6"><div class="form-group"><input type="text" autocomplete="off" name="workexp['+len+'][position]" id="position" class="form-control colorBackground" placeholder="Position" required></div></div><div class="col-md-6"><div class="form-group"><input type="text" autocomplete="off" name="workexp['+len+'][company_name]" id="company_name" class="form-control colorBackground" placeholder="Company name" required></div></div><div class="col-md-3"><div class="form-group"><input type="date" autocomplete="off" name="workexp['+len+'][work_from]" id="work_from'+len+'" class="form-control colorBackground" placeholder="From" onchange="calculateDateDifference('+len+')" required></div></div><div class="col-md-3"><div class="form-group"><input type="date" autocomplete="off" name="workexp['+len+'][work_to]" id="work_to'+len+'" class="form-control colorBackground" placeholder="To" onchange="calculateDateDifference('+len+')" required></div></div><div class="col-md-6"><div class="form-group"><input type="text" autocomplete="off" name="workexp['+len+'][exp_year_month]" id="exp_year_month'+len+'" class="form-control colorBackground" placeholder="Year and month" required></div></div></div></li>');
    });
    //SERVICES OFFER LIST REMOVE - APPEND
    ...
    $(".slots-add-btn").click(function() {
        // Get the parent list item (li) and list (ul) of the clicked "Add" button
        var currentLi = $(this).closest("li");
        var currentUl = currentLi.closest("ul");

        // Get the day value (e.g., Monday, Tuesday, etc.) from the checkbox within the current list item
        var day = currentLi.find("input[type=checkbox]:first-child").val();

        // Calculate the number of existing time slots for the day
        var len = currentUl.children("li").length - 1;

        // Create a new list item (li) with time input fields and append it after the last list item
        var newLi = $('<li class="removeable"> <div class="row"> <div class="col-md-2"> </div><div class="col-md-4"> <input type="time" class="form-control" name="' + day + '[' + len + '][from]" value=""> </div> <div class="col-md-4"> <input type="time" class="form-control" name="' + day + '[' + len + '][to]" value=""> </div>  </div> </li>');
...
if (urlParam("login") == "register") {
   // $('.log-1, .log-3').slideUp();
   // $('.log-2').slideDown(); //satyajeet 25-7-2023
