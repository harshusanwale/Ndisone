...
if (isset($_SESSION['custom_plan_price']) && !empty($_SESSION['custom_plan_price'])) {
    $itemPrice =  $_SESSION['custom_plan_price'];
   }
   else
   {
...
if ($paymenyResponse['amount_refunded'] == 0 && empty($paymenyResponse['failure_code']) && $paymenyResponse['paid'] == 1 && $paymenyResponse['captured'] == 1) {
    ...
    if ($traupres && $paymentStatus == 'succeeded') {
        ...
        if($user_details_row['user_type'] == 'Service provider'){
            $reg_status = $user_details_row['register_status'];
        ...
        $lisupqry = "UPDATE " . TBL . "users
					  SET payment_status = 'Paid',register_status = $reg_status2
        ...
        $_SESSION['status_msg'] = $footer_row['currency_symbol'] . $_SESSION['custom_plan_price']." ".$BIZBOOK['PAYMENT_SUCCESSFULL_USER_UPGRADE_SUCESS'];
