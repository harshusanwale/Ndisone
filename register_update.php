...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['register_submit'])) {
       ...
       // $user_type = $_POST["user_type"];
        ...
        if((!empty($_POST["user_type"]) && $_POST["user_type"] == 1) )
        { $user_type = 'General';}
        elseif(!empty($_POST["user_type"]) && $_POST["user_type"] == 5)
        {  $user_type ='Service provider';}
        elseif(!empty($_POST["user_type"]) && $_POST["user_type"] ==  2)
        {  $user_type ='Participant';}
        elseif(!empty($_POST["user_type"]) && $_POST["user_type"] == 3)
        {  $user_type ='Support Worker';}
        elseif(!empty($_POST["user_type"]) && $_POST["user_type"] == 6)
        {  $user_type ='Support coordinator';}
        elseif(!empty($_POST["user_type"]) && $_POST["user_type"] == 4)
        {  $user_type ='NDIS service experts';}
        else
        { $user_type ='';}
...
*/
                    ...
                    $qry = "INSERT INTO " . TBL . "users
					(first_name, last_name, email_id, mobile_number, password, user_type, user_plan, register_mode, user_address,user_city,user_state,user_country,user_zip_code,user_latitude,user_longitude, profile_image
					, user_points, user_facebook, user_twitter, user_youtube, user_website, user_slug, verification_code, verification_link,verification_status, user_status, user_clear_notification_cdt, user_cdt,register_status)
					VALUES ('$first_name', '$last_name', '$email_id', '$mobile_number', '$password_hash', '$user_type', '$user_plan', '$register_mode',
					 '$user_address','$user_city','$user_state','$user_country','$user_zip_code','$user_latitude','$user_longitude' , '$profile_image', '$user_points', '$user_facebook','$user_twitter', '$user_youtube', '$user_website',
					 '$user_slug','$verification_code','$verification_link','$verification_status','$user_status', '$curDate', '$curDate','$reg_status')";
        ...
        //****************************    Admin email starts    *************************
            ...
            // $mail->SetFrom("$from", "$admin_email");
            // $mail->AddAddress("$to1");
            // $mail->Headers = "$headers1";
            ...
            if ($mode_path == "XeBaCk_MoDeX_PATHXHU") {
                ...
                $_SESSION['status_msg'] = $BIZBOOK['USER_INSERT_SUCCESS_MESSAGE'];  // Success Message in session
                $_SESSION['register_status_msg'] = 1;
                setcookie("user_id", $lastID, time() + (86400 * 30), "/");  
                setcookie("reg_status", $reg_status, time() + (86400 * 30), "/"); 
                // setcookie("post_email_id", $_POST["email_id"], time() + (86400 * 30), "/");
                // setcookie("post_password", md5($_POST["password"]), time() + (86400 * 30), "/");
                // header('Location: login?login=register');
